...
#
# Multi-stage Dockerfile for janusd - File Access Auditing daemon
...
# Copy daemon source
COPY daemons/janusd/c/ ./

# Build janusd with static linking
RUN cmake -B build \
    -DCMAKE_BUILD_TYPE=Release \
    -DBUILD_STATIC=ON \
    -DBUILD_TESTING=OFF \
...
RUN ldd build/janusd 2>&1 | grep -q "not a dynamic executable" || \
    (echo "WARNING: Binary is not fully static:" && ldd build/janusd && exit 0)

# Stage 3: Minimal runtime image - scratch (no OS, just the binary)
FROM scratch

LABEL org.opencontainers.image.title="janusd"
LABEL org.opencontainers.image.description="Janus File Access Auditing Daemon"
LABEL org.opencontainers.image.version="2.0.0"
LABEL org.opencontainers.image.vendor="Como Technologies, LTD"

COPY --from=builder /src/build/janusd /janusd
